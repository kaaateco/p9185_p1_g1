...
GOAL: 
The secondary objective of the trial is to have preliminary assessment and comparison of systemic and local Pharmacokinetics (PK) of Pill A, Gel B, and Gel C. 

Additionally, the investigators are interested to: 1) assess the correlation of PK with adherence measures and the occurrence of adverse events
...
Blood PK ~ Random Intercept (Subject) + Treatment*Timepoint + Carryover + Period
...
```{r}
library(nlme)
fit2 <- lme(bloodVL ~ treatment * timepoint + sequence_ind + period, method="ML",
           data = baseline_long,
          random=reStruct(~ 1 | ptid))
summary(fit2)
```

Skin PK ~ Random Intercept (Subject) + Treatment*Timepoint + Carryover + Period
...
```{r}
fit2 <- lme(skinVL ~ treatment * timepoint + sequence_ind + period, method="ML",
           data = baseline_long,
          random=reStruct(~ 1 | ptid))
summary(fit2)
```
...
ggplot(merged) +
  geom_point(aes(x = bloodVL_change, y = overall_adhere, color = period)) +
  facet_wrap(~ treatment) +
  theme_bw()

ggplot(merged) +
  geom_point(aes(x = bloodVL_change, y = overall_adhere, color = treatment)) +
  theme_bw()

ggplot(merged) +
  geom_point(aes(x = skinVL_change, y = overall_adhere, color = period)) +
  facet_wrap(~ treatment) +
  theme_bw()

ggplot(merged) +
  geom_point(aes(x = skinVL_change, y = overall_adhere, color = treatment)) +
  theme_bw()
...
```{r}
fit2 <- lme(skinVL_change ~ treatment + overall_adhere + sequence_ind + period, method="ML",
           data = merged,
          random=reStruct(~ 1 | ptid))
summary(fit2)


fit2 <- lme(bloodVL_change ~ treatment + overall_adhere + sequence_ind + period, method="ML",
           data = merged,
          random=reStruct(~ 1 | ptid))
summary(fit2)
```
...
```{r}
ggplot(merged, aes(x=bloodVL_change, y=overall_safety, color=treatment)) +
  geom_jitter(height=0.05, alpha=0.3) +
  geom_smooth(method='glm', method.args=list(family='binomial')) +
  theme_bw()

ggplot(merged, aes(x=skinVL_change, y=overall_safety, color=treatment)) +
  geom_jitter(height=0.05, alpha=0.3) +
  geom_smooth(method='glm', method.args=list(family='binomial')) +
  theme_bw()
```
