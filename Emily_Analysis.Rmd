...
\alpha_{ijk}:& \text{adverse event indicator}\\
\beta_{ijk}:& \text{total days of adherence (out of 28)}\\
...
# model_large <- lmer(bloodVL_change ~ treatment + sequence_ind + period + (1|ptid), data = merged)
# 
# model <- lmer(bloodVL_change ~ treatment + period + (1|ptid), data = merged)
# 
...
# model_large <- lmer(skinVL_change ~ treatment + sequence_ind + period + (1|ptid), data = merged)
# summary(model_large)
# 
# model <- lmer(skinVL_change ~ treatment + period + (1|ptid), data = merged)
# 
# model_comparison <- anova(model, model_large)
# print(model_comparison)
# 
...
# 
# 
# model <- lm(skinVL_change ~ treatment + period, data = merged)
# tbl_regression(model) %>%
#        modify_column_unhide(std.error)

#model <- lmer(skinVL_change ~ treatment + period + sequence_ind + (1|ptid), data = merged)
# singular
#summary(model)
...
p2 <- ggplot(data.frame(resid = resid_scaled), aes(x = resid)) +
  ...
  labs(title = "Histogram: Scaled Residuals",
       x = "Scaled Residuals",
       y = "Density",
       caption = "Blue line = Kernel density") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
#           random=reStruct(~ 1 | ptid))
# 
# model_large2 <- lme(bloodVL_change ~ treatment*overall_adhere + 
#                       treatment*overall_safety, method="ML",
#            data = merged,
#           random=reStruct(~ 1 | ptid))
# 
...
<!-- # Get the range of adherence values -->
<!-- adhere_range <- range(merged$overall_adhere, na.rm = TRUE) -->
<!-- adhere_seq <- seq(adhere_range[1], adhere_range[2], length.out = 50) -->

<!-- # Get unique safety levels -->
<!-- safety_levels <- unique(merged$overall_safety) -->
<!-- pred_data <- expand.grid(treatment = c("Pill A", "Gel B", "Gel C"), -->
<!--                          overall_adhere = adhere_seq, -->
<!--                          overall_safety = safety_levels) -->
<!-- pred_data$predicted <- predict(model, newdata = pred_data, level = 0) -->
<!-- emm_grid <- emmeans(model, ~ treatment * overall_adhere * overall_safety, -->
<!--                     at = list(overall_adhere = adhere_seq)) -->
<!-- pred_df <- as.data.frame(emm_grid) -->

<!-- pred_df_labeled <- pred_df %>% -->
<!--   rename(predicted = emmean, se = SE) %>% -->
<!--   mutate(safety_label = ifelse(overall_safety == 0,  -->
<!--                                 "No Safety Issues (safety = 0)",  -->
<!--                                 "Safety Issues (safety = 1)")) -->

<!-- p3 <- ggplot(pred_df_labeled, aes(x = overall_adhere, y = predicted,  -->
<!--                                    color = treatment, fill = treatment)) + -->
<!--   geom_line(linewidth = 1.2) + -->
<!--   geom_ribbon(aes(ymin = predicted - 1.96 * se,  -->
<!--                   ymax = predicted + 1.96 * se), -->
<!--               alpha = 0.2, color = NA) + -->
<!--   facet_wrap(~ safety_label, ncol = 2) + -->
<!--   labs(title = "Treatment Effects: Adherence × Safety Interaction", -->
<!--        x = "Overall Adherence", -->
<!--        y = "Predicted Viral Load Reduction", -->
<!--        color = "Treatment", -->
<!--        fill = "Treatment") + -->
<!--   theme_bw() + -->
...
<!--   scale_color_manual(values = c("Pill A" = "#E41A1C",  -->
<!--                                  "Gel B" = "#377EB8",  -->
<!--                                  "Gel C" = "#4DAF4A")) + -->
<!--   scale_fill_manual(values = c("Pill A" = "#E41A1C",  -->
<!--                                 "Gel B" = "#377EB8",  -->
<!--                                 "Gel C" = "#4DAF4A")) -->
...
<!-- #  -->
<!-- # # Save plots -->
<!-- ggsave("interaction_by_safety.png", p3, width = 12, height = 6, dpi = 300) -->
