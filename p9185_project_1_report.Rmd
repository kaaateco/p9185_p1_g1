...
overall_endpoints <- endpoints %>%
  mutate(study_sequence = paste(period1, paste(period2, period3, sep = "_"), sep = "_")) |> 
...
endpoint_clean_dat <- overall_endpoints |> 
  mutate(carryover = factor(case_when(
      ...
      adverse_event = case_when(
        overall_safety == 0 ~ "No",
        overall_safety == 1 ~ "Yes"),
      adverse_event = as.factor(adverse_event),
...
### Proportion of Adverse Events per period
```{r}
...
anova(glmm2,glmm1) |> 
  broom::tidy() |> 
  kable(caption = "LRT Between Model w/ and w/o Interaction Between Tx and Period", 
                 booktabs = TRUE, longtable = FALSE, linesep = "", digits = 2)  |> 
  kable_styling(latex_options = "hold_position", font_size = 14)
##not significant deviation between the model with and without interaction
```

```{r}
glmm3 <- glmer(adverse_event ~ treatment + period + age + (1 | ptid), 
               family = binomial, data = endpoint_clean_dat, nAGQ = 0)
anova(glmm3,glmm2) |> 
  broom::tidy() |> 
  kable(caption = "LRT Between Model w/ and w/o Race, Gender and Carryover Effect",  booktabs = TRUE, longtable = FALSE, linesep = "", digits = 2)  |> 
  kable_styling(latex_options = "hold_position", font_size = 14)
##not significant deviation between the model with and without gender, race and period and removing carry over effect as there's none
final_mod <- glmm3
```

### Model Result (with Carryover Effect)
```{r}
...
### Final Model Result (without Carryover Effect)

**Notation**: 
$$
\begin{aligned}
\widehat{logit}(P(AE_{ijk} = 1)) &= b_{k} + \mu +\tau_{i} +\pi_j + \beta{a_{k}},\\ 
&\quad i = 1, 2, 3,\ j = 1, 2, 3,\ k = (1, 2, \ldots, 180),\\
&\quad b_{k} \sim N(0, {{\sigma}^2}_b)
\end{aligned}
$$
...
baseline <- read.csv("baseline.csv") %>%
  dplyr::select(-X) %>%
  mutate(sequence_ind = factor(case_when(
...
baseline_long <- baseline %>%
  ...
  dplyr::select(-c(bviral0:sviral6)) %>%
...
overall_endpoints <- endpoints %>%
  ...
  mutate(
    ...
    overall_adhere = case_when(
      ...
      treatment == "Gel B" ~ Adhere_gelB_total,
      treatment == "Gel C" ~ Adhere_gelC_total)) %>%
  dplyr::select(-c(AE_pillA_week1:Adhere_gelC_week4), -c(AE_pillA_total:Adhere_gelC_total))

merged <- baseline_long %>%
  filter(timepoint == 0) %>%
  dplyr::select(-timepoint) %>% 
  rename(baseline_bloodVL = bloodVL,
         baseline_skinVL = skinVL) %>%
  left_join(overall_endpoints %>% dplyr::select(-treatment), by = c("ptid", "period")) %>%
  mutate(overall_safety = factor(ifelse(overall_safety == 1, "Adverse Event", "No Adverse Event"), 
         levels = c("No Adverse Event", "Adverse Event")))
...
```{r, fig.pos='H', results='asis', eval = FALSE}

baseline_by_seq <- baseline %>%
...
```{r}
# model_large_blood_safety <- lmer(bloodVL_change ~ treatment + overall_safety + sequence_ind +
#                       period + baseline_bloodVL + (1 | ptid),
#               data = merged)
model_blood_safety <- lmer(bloodVL_change ~ treatment + overall_safety + period + baseline_bloodVL + (1 | ptid),
              data = merged)
# anova(model_blood_safety, model_large_blood_safety)
# summary(model_blood_safety)

# model_large_skin_safety <- lmer(skinVL_change ~ treatment + overall_safety + sequence_ind +
#                       period + (1 | ptid),
#               data = merged)
model_skin_safety <- lmer(skinVL_change ~ treatment + overall_safety + period + baseline_skinVL + (1 | ptid),
              data = merged)
# anova(model_skin_safety, model_large_skin_safety)
# summary(model_skin_safety)
```
...
tbl_merge(
  ...
  modify_table_body(~.x %>% 
      mutate(label = case_when(
          ...
          label == "overall_adhere" ~ "Additional Day of Adherence",
          label == "overall_safety" ~ "Adverse Event",
          label == "baseline_skinVL" ~ "Baseline Skin VL",
          TRUE ~ label)) %>%
      filter(!is.na(label))) %>%
  modify_header(label ~ "**Comparison**", estimate ~ "**Beta**") %>%
  bold_p(t = 0.05)),
  ...
  modify_caption("Linear mixed-effects model results examining change in blood and skin viral loads (baseline minus end of period) by treatment group, adverse event status, study period, and baseline viral load. Results show beta coefficients, 95% confidence intervals, and p-values for each predictor.")

```


**Model Diagnostics: Blood VL**


```{r}
resid_scaled <- residuals(model_blood_safety, scaled = TRUE)
fitted_vals <- fitted(model_blood_safety)
random_intercepts <- ranef(model_blood_safety)$ptid[, 1]
model_data <- model_blood_safety@frame
...
**Model Diagnostics: Skin VL**

```{r}
resid_scaled <- residuals(model_skin_safety, scaled = TRUE)
fitted_vals <- fitted(model_skin_safety)
random_intercepts <- ranef(model_skin_safety)$ptid[, 1]
model_data <- model_skin_safety@frame
...
```{r}
# model_large_blood_adherence <- lmer(bloodVL_change ~ treatment + overall_adhere + sequence_ind +
#                       period + baseline_bloodVL + (1 | ptid),
#               data = merged)
model_blood_adherence <- lmer(bloodVL_change ~ treatment + overall_adhere + period + baseline_bloodVL + (1 | ptid),
              data = merged)
# anova(model_blood_adherence, model_large_blood_adherence)
# summary(model_blood_adherence)

# model_large_skin_adherence <- lmer(skinVL_change ~ treatment + overall_adhere + sequence_ind +
#                       period + (1 | ptid),
#               data = merged)
model_skin_adherence <- lmer(skinVL_change ~ treatment + overall_adhere + period + baseline_skinVL + (1 | ptid),
              data = merged)
# anova(model_skin_adherence, model_large_skin_adherence)
# summary(model_skin_adherence)
```
...
```{r}
resid_scaled <- residuals(model_blood_adherence, scaled = TRUE)
fitted_vals <- fitted(model_blood_adherence)
random_intercepts <- ranef(model_blood_adherence)$ptid[, 1]
model_data <- model_blood_adherence@frame
...
```{r}
resid_scaled <- residuals(model_skin_adherence, scaled = TRUE)
fitted_vals <- fitted(model_skin_adherence)
random_intercepts <- ranef(model_skin_adherence)$ptid[, 1]
model_data <- model_skin_adherence@frame
