...
j:& \text{Treatment}, j=1,2,3\\
i:& \text{period}, i=1,2,3\\
\pi_i:& \text{period effect}\\
\beta:& \text{Demographic effect}\\
\tau_{ij}:& \text{treatment effect}\\
\lambda_{j}:& \text{sequence/carryover effect}\\
b_{ik}:& \text{subject-specific intercepts}\\
...
plot_model(final_mod, type = "est", transform = NULL) +
  geom_hline(yintercept = 0, color = "black", linetype = 2, linewidth = 0.5) +
  theme_classic()
#dev.off()
```
...
ggplot(a, aes(x = value))+
  ...
  #geom_vline(xintercept = (log_odds_ci[1,] |> pull(`LB`) |> exp()), color = "black", linetype = 2, linewidth = 0.5)+
  #geom_vline(xintercept = (log_odds_ci[1,] |> pull(`UB`) |> exp()), color = "black", linetype = 2, linewidth = 0.5)+
